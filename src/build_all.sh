...
grep "^ *__version__ *=" "googleclouddebugger/version.py" | \
    grep -Eo "[0-9.]+" > "version.txt"
...
echo "Building the debugger agent for Python 3.6"
python3.6 setup.py bdist_wheel
PACKAGE_NAME="google_python_cloud_debugger-${AGENT_VERSION}-cp36"
WHL_FILENAME="${PACKAGE_NAME}-cp36m-linux_x86_64.whl"
auditwheel repair "dist/${WHL_FILENAME}" --plat manylinux2010_x86_64 --only-plat

echo "Building the debugger agent for Python 3.7"
python3.7 setup.py bdist_wheel
PACKAGE_NAME="google_python_cloud_debugger-${AGENT_VERSION}-cp37"
WHL_FILENAME="${PACKAGE_NAME}-cp37m-linux_x86_64.whl"
auditwheel repair "dist/${WHL_FILENAME}" --plat manylinux2010_x86_64 --only-plat

echo "Building the debugger agent for Python 3.8"
python3.8 setup.py bdist_wheel
PACKAGE_NAME="google_python_cloud_debugger-${AGENT_VERSION}-cp38"
WHL_FILENAME="${PACKAGE_NAME}-cp38-linux_x86_64.whl"
auditwheel repair "dist/${WHL_FILENAME}" --plat manylinux2010_x86_64 --only-plat

echo "Building the debugger agent for Python 3.9"
python3.9 setup.py bdist_wheel
PACKAGE_NAME="google_python_cloud_debugger-${AGENT_VERSION}-cp39"
WHL_FILENAME="${PACKAGE_NAME}-cp39-linux_x86_64.whl"
auditwheel repair "dist/${WHL_FILENAME}" --plat manylinux2010_x86_64 --only-plat
