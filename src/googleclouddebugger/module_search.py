...
def FindMatchingFiles(location_path):
  ...
  # The set of module/package path w/ no extension.
  ...
  for module in sys.modules.values():
    ...
    # Note that __path__ only exist for packages and is a list of abs paths.
  ...
  while search_paths:
    ...
    for unused_importer, mod_name, mod_ispkg in pkgutil.iter_modules([path]):
      ...
      if (src_ispkg, src_name) == (mod_ispkg, mod_name):
        AddCandidate(mod_path)
